...
const toggleSubscription = asyncHandler(async (req, res) => {
    ...
    //step1: get subscriberId from req.user_id (set by verifyJWT middleware)
...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
    ...
    res
    .status(200)
    .json(new ApiResponse(200, "Subscribers fetched Succesfully ", subscribers));
})
...
const getSubscribedChannels = asyncHandler(async (req, res) => {
    ...
    const subscribedChannels= await Subscription.find({subscriberId:subscriberId})
    .populate("channelId","username email");        
    //step3 return the response
    res
    .status(200)
    .json(new ApiResponse(200, "Subscribed channels fetched Succesfully ", subscribedChannels));
