...
const createTweet = asyncHandler(async (req, res) => {
    ...
    // owner from req.user_id (from verifyJWT middleware)
    //save tweet to db
    ...
    const newtweet=await Tweet.createTweet({
        content,
        owner: req.user_id  
    })
...
const updateTweet = asyncHandler(async (req, res) => {
    ...
    const {tweertId}= req. params;
    const {content}= req.body;
    
    const tweet = await Tweet.findById(tweertId);
    if(!tweet){
        throw new ApiError(404,"Tweet not found");
    }

    if(tweet.user.toString() !== req.user_id){
...
const deleteTweet = asyncHandler(async (req, res) => {
    ...
    //validate owner
    if((tweet.owner.toString() !==req.user_id)){
        throw new ApiError(403, "You are not allowed to deete this  tweet");
    }
        await tweet.remove();
        res
        .status(200)
